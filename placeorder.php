<?php
	...
	if(isset($_POST['place']))
	{
		include("db.php");
						...
						$query21->execute();
						$query0=$con->prepare("
						UPDATE petdetails AS p
						INNER JOIN addcart AS op ON p.pet_id = op.pet_id
						SET p
						.pet_quantity = p.pet_quantity - op.qty");
...
?>
	<div id="show">
	<h1 style="width:100%;height:40px;line-height:40px;background:#D85587;color:#fff;text-align:center;margin-top:20px;border-radius:4px;text-shadow: 5px 5px 5px #000;box-shadow: 3px 3px 3px #000;">order Details</h1>
				<table>
				<tr>
					<th>S.No</th>
					<th>Pet Name</th>
...
<tr><td colspan="5"><form method="post"><input type="submit" name="place" value="placeOrder" style="padding:10px;border:1px solid #D85587;color:#D85587;background:#fff;border-radius:4px;"></form></td></tr>			
		</table>
	</div>
		<div id="show">
			
		<h1 style="width:95%;text-align:center;background:#D85587;color:#fff;height:50px;line-height:50px;margin-left:30px;margin-top:10px;border-radius:4px;margin-bottom:10px;">Order Details table </h1>
				<table>
				<tr>
					<th>S.No</th>
					<th>Pet Name</th>
